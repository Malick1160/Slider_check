...
int main(int argc, char * argv[])
{
	...
	Result res_3d, res_vol, mcu_init, dsp_init, res_volmax, res_volmin, res_headphone;
	bool hasHeadphones;
	...
	if (R_FAILED(dsp_init)) {//check if dspInit fails
		printf("dspInit Failed! (How did this happen?)\n\nCode: 0x%08lX", dsp_init);
		sleep(15);
		// Exit services
		mcuHwcExit();
		gfxExit();
	...
	while (aptMainLoop())
	{
		...
		res_headphone = DSP_GetHeadphoneStatus(&hasHeadphones);//check if headphone jack is inserted
		...
		if (R_FAILED(res_headphone)) {//check if it fails to detect headphone data
			printf("Failed to read headphone jack data!\n\n(How did this happen?)\n\nCode: 0x%08lX", res_vol);
			sleep(15);
			break;
		}

		if (R_FAILED(res_vol)) {//the following 4 checks all involve the MCU, so if one of them fails it's very bad
		...
		"Volume slider position: %3u %02X \e[K\r\n\n" "3d slider position: %3u %02X \e[K\r\n\n" "Headphones: %s\e[K\r\e[11A",
		volmax, volmax, volmin, volmin, i2c_volslider, i2c_volslider, i2c_3dslider, i2c_3dslider, hasHeadphones ? "TRUE" : "FALSE");
